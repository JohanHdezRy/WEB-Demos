    ...
    <>
      ...
      >
        ...
        <button
          ...
          aria-label="Menu"
        >
          <span className="block w-5 h-[1.5px] bg-white transition-transform duration-300" style={{ transform: menuOpen ? "translateY(6.5px) rotate(45deg)" : "none" }} />
          <span className="block w-5 h-[1.5px] bg-white transition-opacity duration-200" style={{ opacity: menuOpen ? 0 : 1 }} />
          <span className="block w-5 h-[1.5px] bg-white transition-transform duration-300" style={{ transform: menuOpen ? "translateY(-6.5px) rotate(-45deg)" : "none" }} />
        </button>
      </nav>
      {menuOpen && (
        <div className="fixed inset-0 z-40 bg-black/95 flex flex-col items-center justify-center gap-10">
          <button onClick={() => setMenuOpen(false)} className="absolute top-6 right-6 text-white text-2xl bg-transparent border-none cursor-pointer">✕</button>
          {NAV_LINKS.map((l) => (
            <a key={l} href="#" onClick={() => setMenuOpen(false)} className="text-white font-[var(--font-cormorant)] text-4xl font-light tracking-widest uppercase hover:opacity-60 transition-opacity">
              {l}
            </a>
          ))}
        </div>
