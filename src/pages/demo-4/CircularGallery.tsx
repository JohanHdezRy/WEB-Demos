...
function autoBind(instance: Record<string, unknown>): void {
  ...
  Object.getOwnPropertyNames(proto).forEach(key => {
    if (key !== 'constructor' && typeof instance[key] === 'function') {
      instance[key] = (instance[key] as (...a: unknown[]) => unknown).bind(instance)
    }
  })
}
...
class App {
  ...
  createMedias(items: AppConfig['items'], bend = 1, textColor: string, borderRadius: number, font: string) {
    const defaultItems = Array.from({ length: 8 }, (_, i) => ({
      image: `https://picsum.photos/seed/${i + 1}/800/600?grayscale`,
      text: `Track ${String(i + 1).padStart(2, '0')}`,
    }))
    ...
    this.medias = doubled.map((data, index) => new Media({
      ...
      index, length: doubled.length, renderer: this.renderer, scene: this.scene,
  ...
  onWheel(e: Event) {
    ...
    const delta = we.deltaY || (we as unknown as Record<string, number>).wheelDelta
  ...
  addEventListeners() {
    ...
    // Scope wheel events to container so page scroll still works
...
export default function CircularGallery({ items, bend = 3, textColor = '#ffffff', borderRadius = 0.05, font = 'bold 28px sans-serif', scrollSpeed = 2, scrollEase = 0.05 }: CircularGalleryProps) {
