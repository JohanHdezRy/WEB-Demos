...
export function FuzzyText({
  ...
  const resolvePx = useCallback(
    (value: number | string, el: HTMLElement): number => {
    ...
    const draw = () => {
      ...
      for (let row = 0; row < canvas.height; row++) {
        const shift = Math.round(
          (Math.random() - 0.5) * 2 * fuzzRange * intensity * dpr,
        );
  ...
  const handleEnter = () => {
    if (enableHover) hoverRef.current = true;
  };
  const handleLeave = () => {
    if (enableHover) hoverRef.current = false;
  };
