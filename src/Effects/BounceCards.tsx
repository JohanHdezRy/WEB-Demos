...
export default function BounceCards({
  ...
  const getPushedTransform = (base: string, offsetX: number) => {
    ...
    if (m) return base.replace(/translate\(([-0-9.]+)px\)/, `translate(${parseFloat(m[1]) + offsetX}px)`);
  ...
  const pushSiblings = (hoveredIdx: number) => {
    ...
    images.forEach((_, i) => {
      ...
      if (i === hoveredIdx) {
        ...
        gsap.to(sel, { transform: getPushedTransform(transformStyles[i] || 'none', i < hoveredIdx ? -160 : 160), duration: 0.4, ease: 'back.out(1.4)', delay: Math.abs(hoveredIdx - i) * 0.05, overwrite: 'auto' });
      }
    });
  };
  ...
  const resetSiblings = () => {
    ...
    const q = gsap.utils.selector(containerRef);
    images.forEach((_, i) => {
      gsap.to(q(`.card-${i}`), { transform: transformStyles[i] || 'none', duration: 0.4, ease: 'back.out(1.4)', overwrite: 'auto' });
    });
  };
    ...
    >
      {images.map((src, idx) => (
        <div
          ...
          style={{ boxShadow: '0 4px 10px rgba(0,0,0,0.3)', transform: transformStyles[idx] || 'none' }}
